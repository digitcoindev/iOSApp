...
extension NIS: TargetType {
    ...
    var parameters: [String: AnyObject]? {
        ...
        case .AllTransactions(let accountAddress):
            return ["address": accountAddress]
        case .UnconfirmedTransactions(let accountAddress):
            return ["address": accountAddress]
        case .AnnounceTransaction(let requestAnnounce):
            return ["data": requestAnnounce.data, "signature": requestAnnounce.signature]
        }
    }
    ...
    var sampleData: NSData {
        switch self {
        case .Heartbeat:
            return "{\"code\":1,\"type\":2,\"message\":\"ok\"}".UTF8EncodedData
        case .SynchronizeTime:
            return "{\"sendTimeStamp\":43429428889,\"receiveTimeStamp\":43429428889}".UTF8EncodedData
        case .AccountData(let accountAddress):
