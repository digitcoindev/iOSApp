...
enum NEM {
    ...
    case transactions(accountAddress: String, server: Server?)
...
extension NEM: TargetType {
    ...
    var baseURL: URL {
        ...
        case .transactions(_, let server), .unconfirmedTransactions(_, let server):
            if server != nil {
                return server!.fullURL()
            } else {
    ...
    var parameters: [String: Any]? {
        switch self {
        case .accountData(let accountAddress), .ownedMosaics(let accountAddress), .transactions(let accountAddress, _), .unconfirmedTransactions(let accountAddress, _):
    ...
    var sampleData: Data {
        ...
        case .transactions(let accountAddress, _):
